...
for i in range(3):
    ...
    ax0.set(xlabel='$\\rho [\si{\micro\metre}]$', title='Incident Beam Profile',
    ...
    colorbar(im1)
    ax1.set_title('$|\\textbf{E}_{r:xy}|^2$'), ax1.set_ylabel('$r [\lambda]$')
    ...
    im2 = ax2.imshow(BeamResults[6*i+1], cmap = 'inferno', 
                     extent=[-x*4, x*4, -y*4, y*4],  
                             interpolation = interp)
    colorbar(im2)
    ax2.set_title('$|\\textbf{E}_{\phi:xy}|^2$')
    ...
    plt.savefig(fname = 'fields' + str(i),
                bbox_inches = 'tight')
    plt.show()
